...
#include <time.h>
#include <errno.h>
#include "syslogger.h" // Brings in the library for logging
...
// Initialization of timespec variable is {seconds, nanoseconds}
...
// Calculates difference of two timespec
// @param start and stop time
// @return double precision floating point difference
...
// Calculates difference of two timespec
// @param const stop, const start
// @param delta, struct storing result
// @return status
int delta_t(const struct timespec *stop, const struct timespec *start, struct timespec *delta_t)
{
...
void main(void)
{
  ...
  if (result_code)
  {
    printf("ERROR; sched_setscheduler result_code is %d\n", result_code);
  ...
  if (result_code)
  {
    printf("ERROR; pthread_create() result code is %d\n", result_code);
